...
**State**: React `useState` holds tasks, assignments (`taskId → assigneeName`), resources, holidays, vacations, progress (% per task), taskStatuses (status label per task), theme, active tab, `confirmDialog` (pending confirmation action), `sessionFileHandle` (`FileSystemFileHandle | null`), `sessionFileName` (suggested filename for the save picker), `saveStatus` (`null | 'saving' | 'saved'` — drives the Quick Save button feedback flash), `fixedStartDates` (`{ [sn]: "YYYY-MM-DD" }` — optional per-task start date floors), `editTaskModal` (`null | sn` — which task the Edit modal is open for), `ganttContextMenu` (`null | { sn, x, y }` — the Gantt right-click context menu), `autoSave` (boolean, persisted to `localStorage`), and `autoSaveState` (`null | 'saving' | 'saved' | 'needs-permission' | 'error'` — drives the Autosave toggle label).
...
**Autosave** (`autoSave` state, persisted in `localStorage["gantt.autoSave"]`): toolbar toggle next to Quick Save, shown only when `showSaveFilePicker` exists. Every data change (the same deps that set `isDirty`) bumps `dataVersion`; an 800 ms debounced effect then calls `autoSaveNow()`, which writes silently to `sessionFileHandle`. It never opens a picker or calls `requestPermission` (both need a user gesture) — if permission isn't `granted` it shows "Autosave paused" until the user clicks Quick Save. Turning autosave on with no handle calls `quickSave()` to pick a file. Overlapping saves are serialized via `autoSaveInFlightRef`/`autoSavePendingRef`, and `isDirty` is cleared only if no change happened during the write.
